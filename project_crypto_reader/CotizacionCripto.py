   ...
   """
    ...
    def __calcular_vwap(self, dataframe):
        dataframe["indicador_vwap"] = VolumeWeightedAveragePrice(high=dataframe['high'], low=dataframe['low'],
                                                      ...
                                                      window=self.intervalo_vwap, fillna=True).volume_weighted_average_price()
        return dataframe
    def obtener_cotizacion(self):
        ...
        crypto_response, last = kraken.api.get_ohlc_data(self.altname, interval=kraken.intervalo_velas[self.intervalo], since=int(self.desde.timestamp()), ascending=True)
        ...
        """
        data_crypto['indicador_vwap'] = (np.cumsum(
            data_crypto.volume * ((data_crypto.high + data_crypto.low + data_crypto.close) / 3)) /
                                         np.cumsum(data_crypto.volume))
        """
